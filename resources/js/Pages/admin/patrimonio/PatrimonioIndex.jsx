...
// ─── Modal Patrimonio ─────────────────────────────────────────────────────────
function PatrimonioModal({ item, onClose }) {
    const isEdit = !!item;
    const hoy    = new Date();
    ...
    const submit = (e) => {
        ...
        if (isEdit) {
            router.post(route('patrimonio.update', item.id), { ...formData, _method: 'PUT' }, options);
        } else {
            router.post(route('patrimonio.store'), formData, options);
        }
    };
...
function PatrimonioCard({ item, onEdit, onDelete, onPreview }) {
...
// ─── Página principal ─────────────────────────────────────────────────────────
export default function PatrimonioIndex({ items }) {
    ...
    const handleDelete = () => {
        setDeleting(true);
        router.delete(route('patrimonio.destroy', deleteTarget.id), {
            onSuccess: () => {
                setDelete(null);
                setDeleting(false);
