...
function MenuModal({ item, onClose }) {
    ...
    const submit = (e) => {
        ...
        if (isEdit) {
            router.post(route('sae.update', item.id), { ...formData, _method: 'PUT' }, options);
        } else {
            router.post(route('sae.store'), formData, options);
        }
    };
            ...
            <div className="relative w-full max-w-lg rounded-xl bg-white shadow-2xl">
                ...
                <form onSubmit={submit} className="max-h-[80vh] overflow-y-auto px-6 py-4 space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2 border-t">
                        ...
                        >
                            {processing ? 'Guardando...' : isEdit ? 'Guardar cambios' : 'Crear menú'}
                        </button>
                    </div>
                </form>
            </div>
...
// ─── Tarjeta de Menú ──────────────────────────────────────────────────────────
function MenuCard({ item, onEdit, onDelete, onPreview }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-sm rounded-xl bg-white shadow-2xl p-6">
                <h2 className="text-base font-semibold text-gray-800 mb-2">Eliminar menú</h2>
...
// ─── Página principal ─────────────────────────────────────────────────────────
export default function SaeIndex({ items }) {
    ...
    const handleDelete = () => {
        setDeleting(true);
        router.delete(route('sae.destroy', deleteTarget.id), {
            onSuccess: () => {
                setDelete(null);
                setDeleting(false);
    ...
    return (
        <AuthenticatedLayout
            pageTitle="SAE"
            pageSubtitle="Gestioná los menús PDF del Servicio Alimentario Escolar"
            pageColor="#F97316"
            pageAction={
                <button
            ...
            <div className="p-6">
                {items.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white py-16 text-center">
                        ...
                        <p className="text-sm font-medium text-gray-500">No hay menús cargados</p>
                        <p className="text-xs text-gray-400 mt-1">Hacé clic en "Nuevo menú" para agregar el primero</p>
                    </div>
                ) : (
