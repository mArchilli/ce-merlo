    ...
    const submit = (e) => {
        ...
        const options = {
            ...
            onSuccess: () => {
                reset();
                onClose();
                toast.success(isEdit ? 'Recurso actualizado correctamente.' : 'Recurso creado correctamente.');
            },
            onError: () => toast.error('Error al guardar. Revisá los campos requeridos.'),
        };

        if (isEdit) {
            router.post(route('recursos_humanos.update', item.id), { ...formData, _method: 'PUT' }, options);
        } else {
            router.post(route('recursos_humanos.store'), formData, options);
        }
    };
            ...
            <div className="relative w-full max-w-lg rounded-xl bg-white shadow-2xl">
                ...
                <form onSubmit={submit} className="max-h-[80vh] overflow-y-auto px-6 py-4 space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2 border-t">
                        ...
                        >
                            {processing ? 'Guardando...' : isEdit ? 'Guardar cambios' : 'Crear recurso'}
                        </button>
                    </div>
                </form>
            </div>
...
// ─── Tarjeta de Recurso ───────────────────────────────────────────────────────
function RecursoCard({ recurso, onEdit, onDelete, onPreview }) {
    const mesLabel = recurso.mes
        ? MESES.find(([n]) => n === recurso.mes)?.[1]
        : null;

    return (
        <div className={`flex flex-col rounded-xl bg-white shadow-sm border transition-opacity ${recurso.activa ? 'border-gray-100' : 'border-gray-200 opacity-60'}`}>
            {/* Cabecera PDF */}
            <div className="flex items-center gap-3 rounded-t-xl bg-red-50 px-5 py-4 border-b border-red-100">
                ...
                <span className="flex-1 truncate text-sm font-medium text-red-700">
                    {recurso.archivo_pdf ?? 'Sin archivo'}
                </span>
            </div>

            <div className="flex flex-1 flex-col p-5">
                <div className="flex items-start justify-between gap-2 mb-1">
                    <h3 className="font-semibold text-gray-800 leading-snug">{recurso.titulo}</h3>
                    {!recurso.activa && (
                ...
                {recurso.descripcion && (
                    <p className="mt-1 text-sm text-gray-500 line-clamp-2">{recurso.descripcion}</p>
                )}

                {(mesLabel || recurso.anio) && (
                    <p className="mt-2 text-xs text-gray-400">
                        {[mesLabel, recurso.anio].filter(Boolean).join(' ')}
                    </p>
                )}
...
function DeleteModal({ recurso, onConfirm, onClose, processing }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-sm rounded-xl bg-white shadow-2xl p-6">
                <h2 className="text-base font-semibold text-gray-800 mb-2">Eliminar recurso</h2>
                <p className="text-sm text-gray-500 mb-6">
                    ¿Confirmás que querés eliminar <span className="font-medium text-gray-700">"{recurso.titulo}"</span>?
...
// ─── Página principal ─────────────────────────────────────────────────────────
export default function RecursosHumanosIndex({ recursos }) {
    const [modal, setModal]           = useState(null); // null | 'create' | recurso
    ...
    const handleDelete = () => {
        setDeleting(true);
        router.delete(route('recursos_humanos.destroy', deleteTarget.id), {
            onSuccess: () => {
                setDelete(null);
                setDeleting(false);
            ...
            <div className="p-6">
                {recursos.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white py-16 text-center">
                        ...
                        <p className="text-sm font-medium text-gray-500">No hay recursos cargados</p>
                        <p className="text-xs text-gray-400 mt-1">Hacé clic en "Nuevo recurso" para agregar el primero</p>
                    </div>
                ) : (
                    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                        {recursos.map((recurso) => (
                            <RecursoCard
                                ...
                                recurso={recurso}
                                onEdit={(r) => setModal(r)}
                                onDelete={(r) => setDelete(r)}
                                onPreview={(r) => setPdfPreview(r)}
                            />
                        ))}
                    </div>
                )}
